...
def parse_args():
    ...
    parser.add_argument('--basenames', nargs='+', default=['controller', 'worker'],
                        help="Base names of nodes")
...
def main(command, basenames=('controller', 'worker'), confirm=False):
    ...
    for node in nodes:
        ...
        if idx in EXCEPTIONS:
            ...
            continue
        result = client.call('vm.action', 'terminate', idx)
        print("Terminated", result)


if __name__ == '__main__':
    args = parse_args()
    main(args.command, args.basenames, args.confirm)
